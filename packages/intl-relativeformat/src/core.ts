...
import MessageFormat, {IntlMessageFormat} from 'intl-messageformat';
...
interface IntlRelativeFormat {
  ...
  format(date: ConstructorParameters<typeof Date>[0]): string;
}
...
const RelativeFormat: IntlRelativeFormat = ((
  ...
  return {
    format(date: number, options?: { now?: number }) {
